...
function game_loop()
{
	...
	onLogCheck();
	onTurtleCheck();
	specialCheck(); // For extra life, death in river, or game over
}
...
function drawConstants()
{
    ...
    ctx.drawImage(img, 0, 0, 399, 53, 23, 0, 399, 53); // Title
    ctx.drawImage(img, 0, 53, 399, 60, 0, 53, 399, 60); // End locations
    ctx.drawImage(img, 0, 116, 399, 36, 0, 280, 399, 36); // Middle purple rest location
    ctx.drawImage(img, 0, 116, 399, 36, 0, 490, 399, 36); // Starting purple rest location
    if (remaining_lives > 0)
    	ctx.drawImage(img, 8, 326, 24, 27, 0, 528, 15, 16.875); // Num lives 1
    if (remaining_lives > 1)
        ctx.drawImage(img, 8, 326, 24, 27, 12, 528, 15, 16.875); // Num lives 2
    if (remaining_lives > 2)
        ctx.drawImage(img, 8, 326, 24, 27, 24, 528, 15, 16.875); // Num lives 3
    if (remaining_lives > 3)
        ctx.drawImage(img, 8, 326, 24, 27, 36, 528, 15, 16.875); // Num lives 4
...
function drawTimer()
{
	...
	if (!death) {
		if (elapsedTime>20 && (Math.floor(elapsedTime*4))%2==0)
			ctx.fillStyle="rgb(255, 0, 0)"; // Red timer
		else
			ctx.fillStyle="rgb(0, 255, 0)"; // Green timer
...
function drawFly(img)
{
	...
	if (!fly) {
		if (Math.floor(Math.random()*1000)>985) { // 1.5% chance on any frame that fly will appear
			fly=true;
			flyArray = new Array;
			for (i = 0; i < 5; i++) {
				if (!victorySquare[i]) {
...
function specialCheck()
{
	...
	if (progressUp>6 && logging==false && turtling==false &&!moving && !victory) // Death in river
		frogDeath();
	if (game_over) {
		alert("GAME OVER");
		initialize_game();
