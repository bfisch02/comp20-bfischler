...
function callback1() {
			...
			for (j = 0; str[j]; j++)
			{
				...
				if (stations[i].platformKey1 == str[j].PlatformKey)
				{
					if (contentString=="")
					{
						...
						"Time: " + str[j].Time + " Status: " + str[j].InformationType + '</br>';
					}
					else
					{
						contentString+="Time: " + str[j].Time + " Status: " + str[j].InformationType + '</br>';
	...
	navigator.geolocation.getCurrentPosition(shortest_path);
	Waldo_Carmen();
	}
...
function shortest_path(my_pos)
{
	...
	var contentString =
		...
		'Latitude: ' + my_lat + '</br>' +
		'Longitude: ' + my_long + '</br>' +
		'Closest Station: ' + stations[shortest].stationName + '</br>' +
		'Distance: ' + min_length + '</br>' +
		'   miles';
...
function find_hidden(pos)
{
	...
	var my_long = pos.coords.longitude;
		...
		'Distance: ' + my_distance(my_lat, my_long, str[0].loc.latitude, str[0].loc.longitude)
		+ '</br>' + 'miles';
		...
		'Distance: ' + my_distance(my_lat, my_long, str[1].loc.latitude, str[1].loc.longitude)
		+ '</br>' + 'miles';
